...
namespace GyroEngine::Resources
{
    ...
    void Image::MakeDepthStencil(const Utils::Device::QueueType dstQueue)
    {
        MoveToLayout(VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, dstQueue);
    }

    void Image::MakeDepthStencilReadOnly(Utils::Device::QueueType dstQueue)
    {
        MoveToLayout(VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL, dstQueue);
    }
    ...
    bool Image::CreateImage()
    {
        ...
        if (vmaCreateImage(m_device.GetAllocator(), &imageInfo, &allocInfo, &m_image, &m_allocation, nullptr) !=
            VK_SUCCESS)
        {
            return false;
        }
